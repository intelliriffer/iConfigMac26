    ...
    , QWidget(parent)
{
    ...
    // this->setStyleSheet("background:#00e057");
    ...
    for (int j = 0; j <= (int)stereoLinked; j++) {
        ...
        for (int i = 0; i < nChannelTotal; i++) {
            ...
            checkBox->setStyleSheet(
                "QCheckBox { spacing: 4px; color:#777}"
                "QCheckBox::hover {  color:#99e634}"
                "QCheckBox:checked { color:#ff3355}"
                "QCheckBox::indicator { width: 6px; height: 6px; border: 1px solid #333; border-radius: 3px; }"
                "QCheckBox::indicator:checked { background: #99e634; border-color: #000; }");
    ...
    if (mixerInputNumber == 0) {
                        ...
                        for (int i = 0; i < outputs.size(); i++) {
                            if (outputs.at(i) == channel.second) {
                                ...
                                for (QCheckBox* ck : checkBoxes) {
                                    if (ck->property("whichCheckbox").toInt() == which && ck->property("mixerOutputNumber").toInt() != mixerOutputNumberForCheckBox) {
                                        ck->setChecked(false);
                                    }
                                }
                                break;
        ...
        if (chk != NULL) {
            ...
            if (checked) {
                ...
                for (QCheckBox* ck : checkBoxes) {
                    if (ck->property("whichCheckbox").toInt() != which && ck->property("mixerInputNumber").toInt() == mixerInputNumberForCheckBox) {
                        ck->setChecked(false);
                    }
                }
