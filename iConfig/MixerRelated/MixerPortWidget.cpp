    ...
    , parent(parent)
{
    ...
    connect(signalMapper, &QSignalMapper::mappedInt, this, &MixerPortWidget::linkButtonClicked);
...
void MixerPortWidget::buildChannelWidgets()
{
    ...
    for (int inCh = 1; inCh <= mixerInterface->numberInputs(audioPortID); ++inCh) {
        auto channelWidget = new MixerChannelWidget(device, audioPortID, inCh, mixerOutputNumber, in, this);
        if (inCh % 2) {
    ...
    auto channelWidget = new MixerChannelWidget(device, audioPortID, 0, mixerOutputNumber, out, this);
