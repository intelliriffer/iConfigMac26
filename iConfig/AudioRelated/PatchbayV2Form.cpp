...
void PatchbayV2Form::draw(const QRect& srcRect)
{
    ...
    int count2 = 1;
    for (int section = 1; section <= m_source->numSections(); ++section) {
        const auto sectionSize = QSizeF(rect().width(), dy * m_source->numInputsPerSection(section));
    ...
    m_source->for_each_mixer([&](const device_pair_t& m) {
        ...
        if ((outPort > 0) && (inPort > 0) && (outCh > 0) && (inCh > 0) && (outPort <= m_source->numSections()) && (outCh <= m_source->numOutputsPerSection(outPort)) && (inPort <= m_source->numSections()) && (inCh <= m_source->numInputsPerSection(inPort))) {
            QPointF a, b, c;
...
void PatchbayV2Form::drawLegend(QPainter& painter)
{
    ...
    QRectF drawRect = QRectF(QPointF(size.width() * -0.5f, size.height() * -0.5f), size);
