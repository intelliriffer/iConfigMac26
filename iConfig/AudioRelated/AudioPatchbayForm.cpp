    ...
    , outputSignalMapper(new QSignalMapper)
{
    ...
    this->registeredHandlerIDs[Command::ACK] = device->registerHandler(Command::ACK, ackHandler);
...
void AudioPatchbayForm::outputChannelChanged(int comboBoxID)
{
    ...
    const auto& portChannelPair = choiceToPortChannelPairMap[channelComboBox->currentIndex()];
    ...
    AudioCfgInfo& audioCfgInfo)
{
    ...
    device->for_each<AudioPortInfo>([&](const AudioPortInfo& audioPortInfo) {
        ...
        for (int i = configBlock.minOutputChannels();
            ...
            outputOptions << QString("%1 Ch. %2").arg(QString::fromStdString(audioPortInfo.portName())).arg(QString::number(i));
    ...
    AudioPortInfo& audioPortInfo)
{
    ...
    QFont font = portInfoTreeItem->font(0);
    ...
    int inChannel, int& comboBoxID)
{
    ...
    try {
        ...
        selectedIndex = portToChannelToChoiceMap[block.portIDOfOutput()][block.outputChannelNumber()];
    } catch (...) {
    }
    ...
    comboBoxToInPortIDChannelPairMap[comboBoxID] = std::make_pair(audioPortInfo.portID(), inChannel);
    ++comboBoxID;
}
    ...
    commandData_t _commandData)
{
    ...
    if ((ackData.commandID() == Command::SetAudioPortPatchbay) && (ackData.errorCode() == 0)) {
