...
AudioChannelsControlWidget::AudioChannelsControlWidget(
    ...
    , parent(parent)
    , m_TotalOutputChannelInPort(nTotalOutputChannel)
{
    ...
    // qDebug() << "Connecting init signal mapper";
    //  connect(signalMapper, SIGNAL(mapped(int)),
    //      this, SLOT(linkButtonClicked(int)));
    connect(signalMapper, &QSignalMapper::mappedInt, this, &AudioChannelsControlWidget::linkButtonClicked);
...
void AudioChannelsControlWidget::buildChannelWidgets()
{
    ...
    for (size_t ch = 1; ch <= audioControlFeatureSource->numChannels(); ++ch) {
        auto controlWidget = new AudioFeatureControlWidget(audioControlFeatureSource, audioPortID, ch, device, this, m_TotalOutputChannelInPort);
        if (ch % 2) {
        ...
        if (!(ch % 2)) {
            ...
            qDebug() << "connecting link button odd";

            connect(linkButton, &QPushButton::clicked, signalMapper, qOverload<>(&QSignalMapper::map));

            signalMapper->setMapping(linkButton, ch - 1);
            qDebug() << "isSTEREO EDITABLE: " << audioControlFeatureSource->isStereoLinkEditable(ch);
