    ...
    , ui(new Ui::ICSaveDialog)
{
    ...
    if (currentDevice != NULL) {
        ...
        qDebug() << pID;
        if (pID == DevicePID::MIO10 || pID == DevicePID::MIO || pID == DevicePID::MIO2 || pID == DevicePID::MIO4 || pID == DevicePID::iConnect2Plus || pID == DevicePID::iConnect4Plus) {
...
void ICSaveDialog::checkToggled(bool value)
{ // handle button pre-reqs
    ...
    if (obj == ui->checkAll) {
        ...
        if (obj == ui->checkMidiRemap || obj == ui->checkMidiFilters || obj == ui->checkMidiInfo || obj == ui->checkMidiPortRouting) {
        ...
        } else if ((obj == ui->checkAudioConnections || obj == ui->checkAudioInfo || obj == ui->checkAudioMixer || obj == ui->checkAnalog) && m_MIDIOnly == false) { // Disable Audio Patchbay related control for MDID devices, zx, 2017-04-11
        ...
        if ((obj == ui->checkAudioConnections || obj == ui->checkAudioMixer) && m_MIDIOnly == false) { // Disable Audio Patchbay related control for MDID devices, zx, 2017-04-11
            ui->checkAudioInfo->setChecked(true);
        } else if (obj == ui->checkMidiRemap || obj == ui->checkMidiFilters || obj == ui->checkMidiPortRouting) {
...
{
    QString presetsPath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + "/iConfigMac26//presets";
    QDir::root().mkpath(presetsPath);

    QString fileName = presetsPath + "/" + ui->textEditFileName->text() + MainWindow::extensionForPID(currentDevice->getPID());
