...
// Qt debugging - required for qDebug() in Qt6
...
#ifndef _WIN32
    try {
        m_probeIn = boost::shared_ptr<RtMidiIn>(new RtMidiIn(RtMidi::UNSPECIFIED, "iConnectivity iConfig Input"));
        ...
        m_probeOut = boost::shared_ptr<RtMidiOut>(new RtMidiOut(RtMidi::UNSPECIFIED, "iConnectivity iConfig Output"));
...
#else // __IOS__
#ifdef _WIN32
    boost::shared_ptr<RtMidiIn> midiIn = boost::shared_ptr<RtMidiIn>(new RtMidiIn);
...
#else // __IOS__
#ifdef _WIN32
    boost::shared_ptr<RtMidiOut> midiOut = boost::shared_ptr<RtMidiOut>(new RtMidiOut);
...
vector<string> Communicator::getInPorts()
{
    ...
    for (auto i = 0; i < srcCount; ++i) {
        ...
        if (src != (MIDIEndpointRef)NULL) {
            ...
            if (noErr == MIDIObjectGetStringProperty(static_cast<MIDIObjectRef>(src), kMIDIPropertyDisplayName, &name)) {
...
vector<string> Communicator::getOutPorts()
{
    ...
    for (auto i = 0; i < destCount; ++i) {
        ...
        if (dest != (MIDIEndpointRef)NULL) {
            CFStringRef name = nil;
            if (noErr == MIDIObjectGetStringProperty(static_cast<MIDIObjectRef>(dest), kMIDIPropertyDisplayName, &name)) {
                NSString* nsstr = (__bridge NSString*)name;
                auto str = string([nsstr cStringUsingEncoding:NSASCIIStringEncoding]);
                result.push_back(str);
            } else {
...
bool Communicator::openOutput(unsigned int outPort)
{
    ...
    if (outPort < MIDIGetNumberOfDestinations()) {
        // open output
        MIDIEndpointRef outEndPoint = MIDIGetDestination(static_cast<ItemCount>(outPort));
...
bool Communicator::openAllOutputs()
{
    ...
    for (unsigned int outPort = 0; outPort < getOutCount(); ++outPort) {
        if (!MyAlgorithms::contains(m_midiOut, (int)outPort)) {
            try {
                boost::shared_ptr<RtMidiOut> output = boost::shared_ptr<RtMidiOut>(new RtMidiOut());
                m_midiOut[outPort] = output;
                try {
                    output->openPort(outPort);
                } catch (...) {
...
void Communicator::sendSysex(const Bytes& sysex)
{
    ...
    dispatch_async(_serialQueue, ^{
        ...
        if (currentOutPort < outEndPoints.size() && outEndPoints[currentOutPort] != (MIDIEndpointRef)NULL) {
            ...
            MIDISysexSendRequest* request = (MIDISysexSendRequest*)malloc(sizeof(MIDISysexSendRequest));
    ...
    if ((MyAlgorithms::contains(m_midiOut, (int)currentOutPort)) && m_midiOut.at(currentOutPort)) {
...
void Communicator::parseBytes(BytesIter& beginIter, BytesIter& endIter, DeviceID deviceID)
{
    if (m_parser) {
        ...
        while ((blockStart != endIter) && (blockEnd != endIter)) {
            ...
            if (block[7] == 0 && block[8] == 0 && block[9] == 0 && block[10] == 0 && block[11] == 0) { // serial number is 0. replace w/ ours!
